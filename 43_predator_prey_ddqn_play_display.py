...
class DDQN_agent:
    def __init__(self, state_size, action_size):
        ...
        # These are hyper parameters for the DDQN_agent
...
def main():
    ...
    # Agents generation
    agnt_0 = DDQN_agent(state_size, action_size)
    agnt_1 = DDQN_agent(state_size, action_size)
    agnt_2 = DDQN_agent(state_size, action_size)
    agnt_3 = DDQN_agent(state_size, action_size)
    ...
    pygame.init()
    SURFACE = pygame.display.set_mode((60*11, 60*11))
    ...
    while agnt_0.episode < 5:
        ...
        state_prey = np.zeros((10,10))
        state_predators = np.zeros((10,10))
        ...
        row_idx, col_idx = divmod(init_state[0],11)
        """
        row_idx, col_idx = 5,5 
        ...
        game_prey[1:11,1:11] = state_prey
        ...
        prey_row += 1
        prey_col += 1
        predator_rows = [[1,1,10,10]]
        predator_cols = [[1,10,1,10]]
        ...
        game_arr_frame[1:11,1:11] = np.zeros((10,10))
        ...
        while not done and ep_step < 500:
            ...
            game_prey = np.zeros((12,12))
            game_prey[prey_row][prey_col] = 1
            game_arr = game_arr_frame + game_prey + predator_0 + predator_1 + predator_2 + predator_3
            # print(game_arr)
            
            state_t = game_arr[1:11,1:11]
            ...
            predator_0 = np.zeros((12,12))
            predator_1 = np.zeros((12,12))
            predator_2 = np.zeros((12,12))
            predator_3 = np.zeros((12,12))            
            predator_0[int(predator_rows[0][0])][int(predator_cols[0][0])] = 2
            predator_1[int(predator_rows[0][1])][int(predator_cols[0][1])] = 2
            predator_2[int(predator_rows[0][2])][int(predator_cols[0][2])] = 2
            predator_3[int(predator_rows[0][3])][int(predator_cols[0][3])] = 2
            ...
            next_state_t = game_arr[1:11,1:11]
            ...
            for idx in range(11):
                ...
                SURFACE.blit(brick, (60*idx, 60*10))
            for idx in range(9):
                SURFACE.blit(brick, (0, 60*(idx+1)))
                SURFACE.blit(brick, (60*10, 60*(idx+1)))
            ...
            for row_idx in range(8):
                pygame.draw.line(SURFACE, (0, 0, 255), (60*(row_idx+2), 60*1), (60*(row_idx+2), 60*10))
            for col_idx in range(8):
                pygame.draw.line(SURFACE, (0, 0, 255), (60*1, 60*(col_idx+2)), (60*10, 60*(col_idx+2)))
